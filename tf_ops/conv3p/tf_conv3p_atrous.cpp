...
struct Grid {
	 ...
	 */
	void neighbor(T x, T y, T z, int filter_x, int filter_y, int filter_z, int stride_x, int stride_y, int stride_z, T voxel_size,
				  Array<Alloc, int> &point_index, Array<Alloc, int> &filter_cell, Array<Alloc, int> &filter_cell_count, Array<Alloc, float> &dist_sum, Array<Alloc, float> &dist) {
					  
	    int filter_full_x = (filter_x - 1) * stride_x + 1;
		int filter_full_y = (filter_y - 1) * stride_y + 1;
		int filter_full_z = (filter_z - 1) * stride_z + 1;
		...
		int nx = (int)((filter_full_x + 1) * 0.5);
		int ny = (int)((filter_full_y + 1) * 0.5);
		int nz = (int)((filter_full_z + 1) * 0.5);
			...
			for (int oy = -ny; oy <= ny; ++oy) {
				for (int ox = -nx; ox <= nx; ++ox) {
					...
					for (int k = cell_start[cidx]; k < cell_start[cidx + 1]; ++k) {
						...
						int fx = std::min(filter_full_x - 1, (int)((vx - xmin) / voxel_size));
						int fy = std::min(filter_full_y - 1, (int)((vy - ymin) / voxel_size));
						int fz = std::min(filter_full_z - 1, (int)((vz - zmin) / voxel_size));
						...
						int dy = vy- y;
						int dz = vz - z;
						int d = pow(vx ,2)+pow(vy,2)+pow(vz,2);
						float md = sqrt(d);
						
                       
						dist.append(md); 
						// Good point
						//point_index.data = 1/md;
     ...
     */
	void neighbor_count(T x, T y, T z, int point_index, int filter_x, int filter_y, int filter_z, int stride_x, int stride_y, int stride_z, T voxel_size,
	...
	void neighbor_count(int filter_x, int filter_y, int filter_z, int stride_x, int stride_y, int stride_z, T voxel_size,
						Array<Alloc, int> &count) {
		count.zero();
		for (int i = 0; i < points.size; ++i) {
			T x = points[3 * i + 0];
			...
			neighbor_count(x, y, z, i, filter_x, filter_y, filter_z, stride_x, stride_y, stride_z, voxel_size, count);
		}
	}
...
class Conv3pOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    int stride_x = stride_tensor.flat<int>()(0);
	int stride_y = stride_tensor.flat<int>()(1);
	int stride_z = stride_tensor.flat<int>()(2);
...
#pragma omp parallel for
    ...
    for (int b = 0; b < batch_size; ++b) {
		...
		Array<CpuAlloc, float> dist_sum;
		Array<CpuAlloc, float> dist;
		Array<CpuAlloc, float> dist1;
        ...
        for (int i = 0; i < num_points; ++i) {
			...
			grid.neighbor(x, y, z, filter_x, filter_y, filter_z, stride_x, stride_y, stride_z, voxel_size, point_index, filter_cell, filter_cell_count, dist, dist_sum);
			...
			for (int j = 0; j < point_index.size; ++j) {
				...
				for (int c = 0; c < filter_c_out; ++c) {
					for (int k = 0; k < filter_c_in; ++k) {
						...
						res = input[ii * filter_c_in + k] * (1/(T)dist1[i]); }
						
						output[i * filter_c_out + c] += w * res / (T)dist_s; 
...
#pragma omp parallel for
    ...
    for (int b = 0; b < batch_size; ++b) {
		...
		grid.neighbor_count(filter_x, filter_y, filter_z, stride_x, stride_y, stride_z, voxel_size, neighbor_count);
        ...
        for (int j = 0; j < num_points; ++j) {
			...
			grid.neighbor(x, y, z, filter_x, filter_y, filter_z, stride_x, stride_y, stride_z, voxel_size, point_index, filter_cell, filter_cell_count);
			...
			for (int i = 0; i < point_index.size; ++i) {
				...
				int fx = std::min(filter_full_x - 1, (int)((x - xmin) / voxel_size));
				int fy = std::min(filter_full_y - 1, (int)((y - ymin) / voxel_size));
				int fz = std::min(filter_full_z - 1, (int)((z - zmin) / voxel_size));
