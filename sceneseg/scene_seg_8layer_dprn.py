# -*- coding: utf-8 -*-
"""
Created on Thu Aug  7 20:39:39 2018
...
class PointConvNet:
    ...
    def model(self, points_tensor, input_tensor, is_training=True):
        ...
        skipCon1 = tf.add(relu1, conv3, name = None)
        ...
        skipCon2 = tf.add(relu3, conv5, name = None)
        ...
        feat = tf.concat([relu1, relu2, relu3, relu4, relu5, relu6, relu7, relu8], axis=2)
        stride = tf.constant([1, 1, 1])
        filter_tensor = tf.get_variable("filter" , [3, 3, 3, 72, self.num_class])
        output = conv3p(points_tensor, feat, filter_tensor,  stride, voxel_size);
        relu_output = selu.selu(output)
